...
static const QStringList kListUserAgent = {
...
void ImageLib::CImageReader::loadImage(QImage& image, const ImageLib::stReadParam& param)
{
	...
	else
	{
		...
		const QString userAgent = kListUserAgent[qrand() % (kListUserAgent.size())];
		request.setHeader(QNetworkRequest::UserAgentHeader, userAgent);
		
		QNetworkReply* reply = network.get(request);
		QObject::connect(reply, &QNetworkReply::finished, &loop, &QEventLoop::quit);
		loop.exec();
		qDebug() << "Download Result: " << reply->error() << " User Agent" << userAgent << " --> " << param.fileUrl.toString();
